...
import { saveEmployeeData } from "../actions/employeeAction";
const EmployeeFormContainer = props => {
  ...
  const currentEmp = useSelector(
    state => state.employeeReducer && state.employeeReducer.currentEmp
  );
  const formMode = useSelector(
    state => state.employeeReducer && state.employeeReducer.formMode
  );
  ...
  useEffect(() => {
    let keys = currentEmp && Object.keys(currentEmp);
    if (formMode == "view" && keys && keys.length > 0) {
      setEmployeeForm(currentEmp);
      setAddressList(currentEmp.addressList);
    }
  }, [addressList, employeeForm, currentEmp]);
  ...
  const employeeFormValidator = () => {
    if (!employeeForm.name) {
      ...
      if (isAddressValid) {
        ...
        dispatch(saveEmployeeData(employeeForm));
      }
    }
  };
  return (
